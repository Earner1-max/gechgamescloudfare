...
export function Flappy({ level, objects, levelIndex = 1, devMode = false, resumeGraceSec = 0, initialRunTime = 0, onEnd }: FlappyProps) {
  ...
  useEffect(() => {
    ...
    // Flying bear lives on-screen for exactly 5 wall-clock seconds.
    // Bull chases for 4s — but the world only speeds up AFTER the bull is
    // actually visible on screen (not while it's still off-screen).
    ...
    bearWindow.end = bearWindow.start + 8;
    const combinedWindow: Window | null =
    ...
    const drawSpike = (x: number, y: number) => {
      // New spike sprite: bronze spikes on brick base. Mounted at ceiling
      // (cones pointing DOWN toward the player) or at floor (cones pointing
      // UP toward the player). The sprite is anchored flush to the edge.
      const fromTop = y < H / 2;
      const img = getSpikeImg();
      if (img && img.complete && img.naturalWidth > 0) {
        ctx.save();
        if (fromTop) {
          // Flip vertically so sharp cones point down at the bird.
          ctx.translate(x, 0);
          ctx.scale(1, -1);
          ctx.drawImage(img, -SPIKE_W / 2, -SPIKE_H, SPIKE_W, SPIKE_H);
        } else {
          // Floor-mounted, cones point up toward bird.
          ctx.drawImage(img, x - SPIKE_W / 2, H - SPIKE_H, SPIKE_W, SPIKE_H);
        }
        ctx.restore();
      } else {
        // Fallback if sprite not decoded yet.
        ctx.fillStyle = "#6a4818";
        const baseY = fromTop ? 0 : H - SPIKE_H;
    ...
    const tick = (now: number) => {
      ...
      while (nextIdx < sortedObjs.length && sortedObjs[nextIdx].x_time <= runTime + SPAWN_LEAD_SEC) {
        ...
        let spawnX = W + offsetSec * SCROLL_PX_PER_SEC;
        // Enforce flappy_gtc PIPE_SPACING (220px) between consecutive pipes/polls.
      ...
      active = active.filter((o) => {
        ...
        if (o.obj_type === "pipe") {
          ...
          // Snap-to-edge: spike y is renderer-controlled (top vs bottom),
          // but collision uses the actual rendered band so the bird is only
          // killed when entering the cone area, not the empty middle row.
          const fromTopSp = o.y < 0.5;
          const spY = fromTopSp ? SPIKE_H / 2 : H - SPIKE_H / 2;
          drawSpike(x, fromTopSp ? 0 : H);
          if (Math.abs(bird.x - x) < SPIKE_W / 2 + BIRD_SIZE / 2 &&
              Math.abs(bird.y - spY) < SPIKE_H / 2 + BIRD_SIZE / 2) stop(false);
          ...
          // Hammers removed — render as a rotating blade for any legacy levels.
          drawBlade(x, y, Number(o.props.speed ?? 4));
          const dx = bird.x - x, dy = bird.y - y;
          if (dx * dx + dy * dy < (BIRD_SIZE / 2 + BLADE_R - 4) ** 2) stop(false);
