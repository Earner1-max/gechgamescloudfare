 ...
 *
 * Level timer tiers (seconds):
 *   1-10 → 60, 11-20 → 65, 21-30 → 70, 31-40 → 75, 41-50 → 80,
 *   51-60 → 85, 61-70 → 90, 71-80 → 95, 81-90 → 100, 91-100 → 105.
 *   Beyond level 100 the timer stays at 105.
 */
...
function durationForLevel(idx: number): number {
  const clamped = Math.min(100, Math.max(1, idx));
  const tier = Math.floor((clamped - 1) / 10); // 0..9
  return 60 + tier * 5;
}
...
const SkipInput = z.object({ initData: z.string().min(1).max(4096) });
